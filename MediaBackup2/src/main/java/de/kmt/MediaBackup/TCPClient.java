	...
	public void main(String[] args) {
		...
		try {
			...
			DataOutputStream dos = new DataOutputStream(
					new FileOutputStream(Config.getConfigKey("directory") + Config.getConfigKey("file")));
			...
			while (true) {
				...
				out.write(Config.getConfigKey("file"));
				out.flush();
				...
				while (true) {
					...
					if (readbytes < 32768) {
						...
						if (readbytes > 2) {
							// less than 32768 bytes are read. Check if last 3 bytes are "END". If so: stop
							// receiving data
							if ((buffer[readbytes - 3] == 0x45) & (buffer[readbytes - 2] == 0x4e)
									& (buffer[readbytes - 1] == 0x44)) {
